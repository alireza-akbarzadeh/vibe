...
export const SidebarActions = ({
	...
	return (
		...
		>
			...
			<ActionButton
				icon={
					<Heart
						className={cn(
							"transition-colors",
							video.isLiked ? "fill-rose-500 text-rose-500" : "text-white",
						)}
					/>
				}
			...
			{/* Share Action */}
			<ShareDrawer videoUrl={`${window.location.origin}/reels/${video.id}`} />
			...
			<ActionButton
				icon={
					<Bookmark
						className={cn(
							"transition-colors",
							video.isSaved ? "fill-yellow-400 text-yellow-400" : "text-white",
						)}
					/>
				}
			...
			<ActionButton
				icon={
					<Maximize2
						className={cn(
							"transition-all",
							isFocused ? "text-blue-400 scale-110" : "text-white",
						)}
					/>
				}
