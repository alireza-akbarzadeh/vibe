        ...
        >
            ...
            <div className={cn(
                ...
                <div className="flex items-center gap-3 mb-4">
                    <button onClick={(e) => { e.stopPropagation(); toggleFollow(video.user.username); }} className="relative no-pause">
                        <img src={video.user.avatar} className="size-11 rounded-full border-2 border-white object-cover shadow-xl" alt="" />
                    ...
                    <span className="text-white font-bold text-sm tracking-wide drop-shadow-lg">@{video.user.username}</span>
                </div>
                <p className="mb-4 text-white/95 text-sm font-medium line-clamp-2 leading-snug drop-shadow-md max-w-[90%]">{video.caption}</p>
                <div className="flex items-center gap-2 text-white/80">
                    <Music2 className="size-3.5 animate-pulse" />
                    <div className="overflow-hidden w-48">
                        <motion.p animate={{ x: [0, -150] }} transition={{ duration: 8, repeat: Infinity, ease: "linear" }} className="text-[11px] font-bold uppercase whitespace-nowrap tracking-wider">
