...
export function ReelsDomain() {
	const { videos, activeTab, activeVideoId, commentModalOpen } =
		useStore(reelsStore);
	...
	useEffect(() => {
		if (data) {
				...
				page.data.items
					...
					.map((item) => ({
						...
						user: {
							...
							avatar:
								item.creators[0]?.creator.image ||
								`https://api.dicebear.com/7.x/avataaars/svg?seed=${item.creators[0]?.creator.name || item.id}`,
							isFollowing: false,
							isVerified: true,
						},
			...
			setCurrentVideoIndex((prev) =>
				prev >= mappedVideos.length
					? Math.max(0, mappedVideos.length - 1)
					: prev,
			);
		}
	}, [data]);
	...
	return (
		<main
			vaul-drawer-wrapper=""
			className={`bg-[#0a0a0a] items-center relative flex justify-center ${layoutSize}`}
		>
