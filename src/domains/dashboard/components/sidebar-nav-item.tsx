...
export function SidebarNavItem({ item, isCollapsed, pathname }: { item: SidebarItem, isCollapsed: boolean, pathname: string }) {
                ...
                <Tooltip delayDuration={0}>
                    <TooltipTrigger asChild>
                        <DropdownMenuTrigger asChild>
                            <button className={cn(itemClasses, "cursor-pointer outline-none overflow-visible")}>
                    ...
                    <TooltipContent side="right" sideOffset={12} className="text-xs font-bold z-50">
                        {item.label}
                    </TooltipContent>
                </Tooltip>
                    ...
                    <div className="mt-1 space-y-0.5">
                        {item.children?.map((child) => (
                            <DropdownMenuItem key={child.href} asChild>
                                ...
                                >
                                    {child.label}
                                    {pathname === child.href && <div className="h-1 w-1 rounded-full bg-primary" />}
                                </Link>
                            </DropdownMenuItem>
                        ))}
                    </div>
    ...
    return (
        <Tooltip delayDuration={0}>
            ...
            {isCollapsed && (
                <TooltipContent side="right" sideOffset={10} className="bg-popover border-border px-3 py-1.5 text-xs font-bold shadow-xl z-50">
                    {item.label}
                </TooltipContent>
            )}
        </Tooltip>
