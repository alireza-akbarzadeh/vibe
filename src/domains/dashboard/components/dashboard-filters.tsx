...
import { Calendar as CalendarIcon, Filter, Laptop, LayoutGrid, RotateCcw, ShieldCheck } from "lucide-react";
    ...
    return (
        <Sheet>
            <SheetTrigger asChild>
                <Button variant="outline" className="relative rounded-2xl border-dashed bg-background/50 backdrop-blur-sm">
            ...
            <SheetContent className="w-full sm:max-w-md border-l border-border/40 bg-card/95 backdrop-blur-xl flex flex-col">
                <SheetHeader className="mt-6">
                    <div className="flex items-center justify-between">
                        <SheetTitle className="text-2xl font-bold text-primary">Filters</SheetTitle>
                        <Button
                            variant="ghost"
                            size="sm"
                ...
                <div className="flex-1 overflow-y-auto py-6 space-y-8 pr-2">
                    ...
                    <div className="flex items-center justify-between p-4 rounded-3xl bg-primary/5 border border-primary/10">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="space-y-0.5">
                                ...
                                <p className="text-[11px] text-muted-foreground">Only KYC cleared accounts</p>
                            </div>
                        </div>
                        <Switch
                            checked={filters.verifiedOnly}
                            onCheckedChange={(v) => actions.updateFilters({ verifiedOnly: v })}
                        />
                    </div>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {['Desktop', 'Mobile', 'Tablet'].map(device => (
                                <Button key={device} variant="outline" className="rounded-xl h-8 text-xs">
                                    {device}
                                </Button>
                            ))}
                        </div>
                    ...
                    {/* 5. RISK PROFILE */}
                    <div className="space-y-3">
                        <Label className="text-[10px] font-bold uppercase tracking-tighter text-muted-foreground">Security Risk Level</Label>
                        <div className="grid grid-cols-2 gap-2">
                            {['safe', 'medium', 'high'].map((level) => (
                                <Button
                                    key={level}
                                    variant={filters.riskThreshold === level ? 'default' : 'outline'}
                                    onClick={() => actions.updateFilters({ riskThreshold: level as any })}
