	...
	return (
		<div className="h-24 px-4 flex items-center justify-between gap-4 bg-[#121212] border-t border-white/5">
			...
			<div className="flex items-center gap-4 w-[30%] min-w-0">
				...
				<div className="flex items-center gap-3 ml-2">
					...
					<button
						...
						className="text-[#b3b3b3] hover:text-white transition-colors w-11 h-11 rounded-full flex items-center justify-center bg-white/5 "
					>
			...
			<div className="flex-1 max-w-[45%] flex flex-col items-center">
				...
				<div className="flex items-center gap-2 w-full">
					<span className="text-[11px] text-[#a7a7a7] tabular-nums w-10 text-right">
						{formatPayerTime(currentTime)}
					</span>
					<div
						ref={progressBarRef}
						onClick={(e) =>
							handleSeek(e, progressBarRef, (p) =>
								musicAction.updateCurrentTime(
									Math.floor(p * currentSong.duration),
								),
							)
						}
					...
					<span className="text-[11px] text-[#a7a7a7] tabular-nums w-10">
						{formatPayerTime(currentSong.duration)}
					</span>
				</div>
			</div>
			...
			<div className="flex items-center gap-3 w-[30%] justify-end">
				...
				<button onClick={() => musicAction.toggleQueue(currentSong.id)}>
					<ListMusic
						className={cn(
							"w-4 h-4 transition-colors",
							store.showQueue
				...
				<div className="flex items-center gap-2 w-32 group">
					<button
						onClick={() =>
							musicStore.setState((s) => ({ ...s, isMuted: !s.isMuted }))
						}
					>
						{isMuted || volume === 0 ? (
