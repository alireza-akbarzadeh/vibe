...
export function BottomPlayer({
    ...
    const handleVolumeSeek = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
        ...
        const x = e.clientX - rect.left;
        const percentage = Math.round(Math.max(0, Math.min(1, x / rect.width)) * 100);
            ...
            <div className="h-full flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-4 w-[30%] min-w-50">
                    <div className="relative group">
                        ...
                        />
                        <button type="button" className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity bg-black/60 rounded-full p-0.5">
                ...
                <div className="flex-1 max-w-[45%] flex flex-col items-center">
                    <div className="flex items-center gap-6 mb-2">
                        <button type="button" onClick={toggleShuffle} className={isShuffle ? 'text-purple-500' : 'text-[#b3b3b3] hover:text-white'}>
                            <Shuffle className="w-4 h-4" />
                        </button>
                        <button type="button" className="text-[#b3b3b3] hover:text-white"><SkipBack className="w-5 h-5 fill-current" /></button>
                        ...
                        <button type="button" className="text-[#b3b3b3] hover:text-white"><SkipForward className="w-5 h-5 fill-current" /></button>
                        <button type="button" className="text-[#b3b3b3] hover:text-white"><Repeat className="w-4 h-4" /> </button>
                    </div>

                    {/* Pink/Purple Theme Progress Bar */}
                    <div className="flex items-center gap-2 w-full">
                        <span className="text-[11px] text-[#a7a7a7] tabular-nums w-10 text-right">{formatTime(currentTime)}</span>
                        <div
                            ref={progressBarRef}
                        ...
                        <span className="text-[11px] text-[#a7a7a7] tabular-nums w-10">{formatTime(duration)}</span>
                    </div>
                </div>

                {/* Right: Extra Icons & Volume (As per Screenshot) */}
                <div className="flex items-center gap-3 w-[30%] justify-end">
                    <button type="button" className="text-[#b3b3b3] hover:text-white transition-colors"><Mic2 className="w-4 h-4" /></button>
                    <button type="button" className="text-[#b3b3b3] hover:text-white transition-colors"><ListMusic className="w-4 h-4" /></button>
                    <button type="button" className="text-[#b3b3b3] hover:text-white transition-colors"><MonitorSpeaker className="w-4 h-4" /></button>
                    ...
                    <button type="button" className="text-[#b3b3b3] hover:text-white transition-colors"><Maximize2 className="w-4 h-4" /></button>
                </div>
            </div>
        </motion.div>
    );
