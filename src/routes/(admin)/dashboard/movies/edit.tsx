...
import type { MediaCreatorSchema, MediaGenreSchema } from "@/orpc/models/media.schema";
...
export const Route = createFileRoute("/(admin)/dashboard/movies/edit")({
    ...
    loader: async ({ deps }) => {
        const response = await client.media.find({ id: deps.mediaId });
        ...
        <div className="container mx-auto py-6">
            <MediaForm
                ...
                initialData={{
                    ...
                    creatorIds: media.creators?.map((c: MediaCreator) => c.creator.id) || [],
                }}
            />
        </div>
    );
