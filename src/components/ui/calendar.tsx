		...
		<ContextProvider value={customContext}>
			<DayPicker
				...
				classNames={{
					...
					month_caption:
						"flex justify-center h-7 mx-10 relative items-center pt-1",
...
function Chevron({ orientation }: ComponentProps<CustomComponents["Chevron"]>) {
	const Icon = orientation === "left" ? LucideChevronLeft : LucideChevronRight;
	return <Icon className="size-4" />;
}

function Nav({ className }: ComponentProps<CustomComponents["Nav"]>) {
	const { nextMonth, previousMonth, goToMonth } = useDayPicker();
	const { navView, displayYears, setDisplayYears, dayPickerProps } =
		useContext();
...
function CaptionLabel({
	children,
}: ComponentProps<CustomComponents["CaptionLabel"]>) {
