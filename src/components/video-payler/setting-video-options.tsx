import {
    Check, Gauge, Monitor, PictureInPicture,
    RotateCw, Settings, Subtitles
} from 'lucide-react';
                ...
                <div className="p-6 space-y-8">
                    ...
                    <section>
                        <div className="flex items-center gap-2 mb-4 text-zinc-400">
                            ...
                            <span className="text-xs font-bold uppercase tracking-widest">Speed</span>
                        </div>
                        <div className="grid grid-cols-3 gap-2">
                            {SPEED_OPTIONS.map((s) => (
                                <button
                                    ...
                                    className={`py-2 text-xs rounded-lg border transition-all ${currentSpeed === s.value
                    ...
                    <section>
                        ...
                        <div className="space-y-1">
                            {QUALITY_OPTIONS.map((q) => (
                                ...
                                >
                                    <span className={quality === q ? 'text-white' : 'text-zinc-500 group-hover:text-zinc-300'}>{q}</span>
                                    {quality === q && <Check className="h-4 w-4 text-purple-500" />}
                                </button>
                            ))}
                        </div>
                    </section>
                    ...
                    <section>
                        <div className="flex items-center gap-2 mb-4 text-zinc-400">
                            ...
                            <span className="text-xs font-bold uppercase tracking-widest">Subtitles</span>
                        </div>
                        <div className="flex flex-wrap gap-2">
                            {SUBTITLE_OPTIONS.map((sub) => (
                                <button
                                    ...
                                    className={`px-4 py-2 text-xs rounded-full transition-all ${subtitles === sub
                    ...
                    <div className="space-y-4">
                        ...
                        >
                            <RotateCw className={`h-5 w-5 ${isLooping ? 'text-purple-500' : ''}`} />
                            <span className="flex-1 text-left">Loop Video</span>
                            <div className={`w-8 h-4 rounded-full relative transition-colors ${isLooping ? 'bg-purple-600' : 'bg-zinc-700'}`}>
                                <div className={`absolute top-1 w-2 h-2 bg-white rounded-full transition-all ${isLooping ? 'right-1' : 'left-1'}`} />
                            </div>
                        </button>
