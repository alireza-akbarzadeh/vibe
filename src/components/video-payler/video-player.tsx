...
export function VideoPlayer({
	...
	const [internalSessionId, _setInternalSessionId] = useState<string | null>(
		null,
	);
	...
	useEffect(() => {
		...
		const onPlay = () => {
			webSocketService.send("playback-update", {
				roomId: sessionId,
		...
		webSocketService.on("playback-state", onPlaybackState);

		return () => {
			video.removeEventListener("play", onPlay);
			video.removeEventListener("pause", onPause);
			video.removeEventListener("seeked", onSeeked);
			webSocketService.off("playback-state", onPlaybackState);
		};
	}, [sessionId]);
