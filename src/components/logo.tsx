...
export function Logo() {
	...
	const status = (auth?.user?.subscriptionStatus || "FREE") as SubscriptionStatus;
	const colors = statusColors[status] || statusColors.FREE;

	return (
		<Link to="/" className="flex items-center gap-4 group shrink-0">
			<div className="relative">
				<div className={`absolute inset-0 bg-linear-to-tr ${colors.gradient} blur-2xl opacity-20 group-hover:opacity-60 transition-opacity duration-500`} />
				...
				<div className="relative w-12 h-12 rounded-4xl bg-[#0d0d0f] border border-white/10 flex items-center justify-center shadow-2xl group-hover:scale-105 group-hover:-rotate-3 transition-all duration-500 overflow-hidden">
					...
					>
						<Play className={`w-9 h-9 ${colors.icon} fill-${colors.icon.replace('text-', '')}/10 stroke-[1px]`} />
					</motion.div>
			...
			<div className="flex flex-col">
				...
				<div className="flex items-center gap-2 mt-1.5">
					...
					<span className={`text-[8px] ${colors.badge} font-bold uppercase tracking-widest`}>
						{status}
					</span>
				</div>
			</div>
