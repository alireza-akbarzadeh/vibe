...
} from "@/domains/dashboard/store/dashboard.store";
import {
	actions,
	dashboardStore,
} from "@/domains/dashboard/store/dashboard.store";
		...
		<Popover open={isOpen} onOpenChange={actions.setNotificationOpen}>
			<PopoverTrigger asChild>
				<Button
					size="icon"
					variant="ghost"
					className="h-12 w-12 rounded-full bg-linear-to-r from-primary to-secondary relative hover:bg-muted group"
				>
					<motion.div
						animate={
							totalUnread > 0 ? { rotate: [0, -10, 10, -10, 10, 0] } : {}
						}
						transition={{
			...
			<PopoverContent
				align="end"
				className="w-80 sm:w-96 p-0 rounded-2xl overflow-hidden shadow-2xl border-border/50 bg-card/95 backdrop-blur-xl z-50"
			>
				<Tabs defaultValue="alerts" className="w-full">
					...
					<div className="p-2 bg-muted/20 border-t">
						<Button
							variant="outline"
							className="w-full h-9 text-[10px] font-bold uppercase tracking-widest gap-2 rounded-xl hover:bg-primary hover:text-white transition-all shadow-sm"
						>
...
function NotificationItem({ n, i }: { n: NotificationItemType; i: number }) {
	return (
		<motion.div
			layout
...
function MessageItem({ m, i }: { m: MessageItemType; i: number }) {
	return (
		<motion.div
			layout
